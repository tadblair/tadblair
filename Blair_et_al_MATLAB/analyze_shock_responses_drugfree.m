...
if r<26
    eventtimes=shocktimes;
else
    eventtimes=shocktimes2;
end    
    figure(101); %graph window for shock responses
    if r==rats_to_analyze(1); clf; end
    if r<5
        splot=r;
    elseif r==6
        splot=5;
...
%allshockcells_peakdf = [allshockcells_peakdf; analyze_peakshk1(SHKexc1(:) & shk_isplace1(:))]; 
%allnonshockcells_peakdf = [allnonshockcells_peakdf; analyze_peakshk1(~SHKexc1(:) & shk_isplace1(:))];
...
allnonshockcells_peakdf = [allnonshockcells_peakdf; analyze_peakshk1(~SHKexc(:) & shk_isplace(:))];
allnonshockcells_approachdf=[allnonshockcells_approachdf; approach_1(~SHKexc(:) & shk_isplace(:))']; %initialize cell flags for excitation by shock1

if ~(shockframe1 == shockframe2) & ~(shkdir1 == shkdir2)
   % allshockcells_approachdf=[allshockcells_approachdf; approach_2(SHKexc2(:) & shk_isplace2(:))']; %initialize cell flags for excitation by shock1
   % allnonshockcells_approachdf=[allnonshockcells_approachdf; approach_2(~SHKexc2(:) & shk_isplace2(:))']; %initialize cell flags for excitation by shock1
%    allshockcells_peakdf = [allshockcells_peakdf; analyze_peakshk2(SHKexc2(:) & shk_isplace2(:))]; 
%    allnonshockcells_peakdf = [allnonshockcells_peakdf; analyze_peakshk2(~SHKexc2(:) & shk_isplace2(:))];
    ...
    allnonshockcells_peakdf = [allnonshockcells_peakdf; analyze_peakshk2(~SHKexc(:) & shk_isplace(:))];
    allnonshockcells_approachdf=[allnonshockcells_approachdf; approach_2(~SHKexc(:) & shk_isplace(:))']; %initialize cell flags for excitation by shock1
    %PETH is average of responses to first and second shocks
    %allshockcells_df = [allshockcells_df; (1*[shock1_PETH(SHKexc(:) & shk_isplace(:),:)] + 1*[shock2_PETH(SHKexc(:) & shk_isplace(:),:)])/2 ];
    %allnonshockcells_df = [allnonshockcells_df; (1*[shock1_PETH(~SHKexc(:) & shk_isplace(:),:)] + 1*[shock2_PETH(~SHKexc(:) & shk_isplace(:),:)])/2];

    ncurves=ncurves+sum(SHKexc2(:)); 
end
    %PETH is just response to first significant shock response
    allshockcells_df = [allshockcells_df; 1*shock1_PETH(SHKexc1(:) & shk_isplace(:),:); 1*shock2_PETH(~SHKexc1(:) & SHKexc2(:) & shk_isplace(:),:)];
    allnonshockcells_df = [allnonshockcells_df; 1*shock1_PETH(~SHKexc(:) & shk_isplace(:),:); ];
    
    allshockcells_peakdf = [allshockcells_peakdf; analyze_peakshk1(SHKexc1(:) & shk_isplace(:)); analyze_peakshk2(~SHKexc1(:) & SHKexc2(:) & shk_isplace(:))]; 
    allshockcells_approachdf=[allshockcells_approachdf; approach_1(SHKexc1(:) & shk_isplace(:))'; approach_2(~SHKexc1(:) & SHKexc2(:) & shk_isplace(:))']; %initialize cell flags for excitation by shock1
%end
...
%allshockinhcells_peakdf = [allshockinhcells_peakdf; [analyze_peakshk1(SHKinh(:) & shk_isplace(:)) analyze_peakshk2(SHKinh(:) & shk_isplace(:))]];
%allnonshockcells_peakdf = [allnonshockcells_peakdf; [analyze_peakshk1(~SHKexc(:) & ~SHKinh(:) & shk_isplace(:)) analyze_peakshk2(~SHKexc(:) & ~SHKinh(:) & shk_isplace(:))]];
